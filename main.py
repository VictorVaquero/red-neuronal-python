...
import random
import imp
mnist_loader = imp.load_source('mnist_loader', '/home/victor/repos/neural-networks-and-deep-learning/src/mnist_loader.py')
...
class Red(object):
    ...
    def train(self, data, mini_batch_size, epoch, eta, test = False):
        ...
        for x in range(epoch):
            ...
            print ("Numero finalizado {0}, numero total: {1}".format(x, total))
    ...
    def evaluate(self,test_data):
        ...
        result = [(np.argmax(self.feedforward(x)),np.argmax(y)) for (x,y) in test_data]
